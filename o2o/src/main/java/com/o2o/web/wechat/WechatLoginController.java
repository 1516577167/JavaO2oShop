 ...
 * https://open.weixin.qq.com/connect/oauth2/authorize?appid=wxc8f38839cbe89114&redirect_uri=http://120.79.160.13/o2o/wechatlogin/logincheck&role_type=1&response_type=code&scope=snsapi_userinfo&state=1#wechat_redirect
...
public class WechatLoginController {
	...
	@Autowired
	private PerInfoService personInfoService;
	@Autowired
	...
	public String doGet(HttpServletRequest request, HttpServletResponse response) {
		...
		if (auth == null) {
			...
			WechatAuthExecution we = wechatAuthService.insertWechatAuth(auth);
			if (we.getState() != WechatAuthStateEnum.SUCCESS.getState()) {
				return null;
			} else {
				personInfo = personInfoService.queryPersonInfo(auth.getPersonInfo().getUserId());
